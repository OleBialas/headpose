...
class PoseEstimator:
    ...
    def pose_from_image_aruco(self, image):        
        ...
        params.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_SUBPIX #set parameters for marker detection
        (corners, ids, rejected) = cv2.aruco.detectMarkers(image,        # detect markers
            arucoDict, parameters=params)
        mtx = np.array([[927.79941685,   0.        , 529.33143081],
       ...
       [  0.        ,   0.        ,   1.        ]])
        if len(corners) > 1:
            ...
            rotation_vec, translation_vec, _objPoints = cv2.aruco.estimatePoseSingleMarkers(corners, 0.05, mtx, dist_coeffs)
